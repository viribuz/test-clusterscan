...
const MVATable = () => {
  ...
  useEffect(() => {
    ...
    const savedData = JSON.parse(localStorage.getItem('mvatable_data')) || [];
  ...
  const submitForm = () => {
    ...
    localStorage.setItem('mvatable_data', JSON.stringify(newData));
    ...
    <div className="container-fluid" style={{ paddingLeft: '5rem', marginTop: '2rem', marginBottom: '3rem' }}>
      ...
      <Modal show={modalVisible} onHide={closeModal}>
        <Modal.Header closeButton>
          <Modal.Title style={{ color: 'black', fontWeight: 'bold' }}>Lawty Law MVA</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            {/* Form fields go here */}
