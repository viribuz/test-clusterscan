...
const TALCTable = () => {
  ...
  useEffect(() => {
    ...
    const savedData = JSON.parse(localStorage.getItem('talctable_data')) || [];
  ...
  const submitForm = () => {
    ...
    localStorage.setItem('talctable_data', JSON.stringify(newData));
    ...
    <div className="container-fluid" style={{ paddingLeft: '5rem', marginTop: '2rem', marginBottom: '3rem' }}>
      ...
      <Modal show={modalVisible} onHide={closeModal}>
        <Modal.Header closeButton>
          <Modal.Title style={{ color: 'black', fontWeight: 'bold' }}>TALC</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            {/* Form fields go here */}
